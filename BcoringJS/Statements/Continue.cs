    ...
    public sealed class Continue : CodeNode
    {
        ...
        internal static CodeNode Parse(ParseInfo state, ref int index)
        {
            ...
            if (!state.AllowContinue.Peek())
                ExceptionHelper.Throw((new Bcoring.ES6.BaseLibrary.SyntaxError("Invalid use of continue statement")));
            ...
            if (Parser.ValidateName(state.Code, ref i, state.Strict))
            {
                ...
                if (!state.Labels.Contains(label._oValue.ToString()))
                    ExceptionHelper.Throw((new Bcoring.ES6.BaseLibrary.SyntaxError("Try to continue to undefined label.")));
            }
            int pos = index;
            index = i;
