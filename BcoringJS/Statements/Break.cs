        ...
        internal static CodeNode Parse(ParseInfo state, ref int index)
        {
            ...
            if (Parser.ValidateName(state.Code, ref i, state.Strict))
            {
                ...
                if (!state.Labels.Contains(label._oValue.ToString()))
                    ExceptionHelper.Throw((new Bcoring.ES6.BaseLibrary.SyntaxError("Try to break to undefined label.")));
            }
            else if (!state.AllowBreak.Peek())
                ExceptionHelper.Throw((new Bcoring.ES6.BaseLibrary.SyntaxError("Invalid use of break statement")));
            var pos = index;
            index = i;
            state.BreaksCount++;
            return new Break()
