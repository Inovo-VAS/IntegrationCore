...
namespace Retriever
{
    ...
    {
        private const string mimetypesvls = @"F3D Crossword Plugin|application/vnd.hzn-3d-crossword|.x3d|IANA: 3D Crossword Plugin
...
Zzazz Deck|application/vnd.zzazz.deck+xml|.zaz|IANA: Zzazz@";
        ...
        private static Dictionary<string, string> mappedExtentions = new Dictionary<string, string>();
        private static Dictionary<string, bool> mappedTextExtentions = new Dictionary<string, bool>();

        public static bool TextExtension(string ext, bool defaulState = false)
        {
            var extState = defaulState;
            if (ext.LastIndexOf(".") >= 0)
            {
                ext = ext.Substring(ext.LastIndexOf("."));
                if (!ext.Equals("."))
                {
                    lock (mappedTextExtentions)
                    {
                        if (!mappedTextExtentions.ContainsKey(ext))
                        {
                            mappedTextExtentions.Add(ext, extState = ",.html,.html,.xml,.js,.css,.svg,.json,.txt,.csv,".Contains("," + ext + ","));
                        }
                        else
                        {
                            extState = mappedTextExtentions[ext];
                        }
                    }
                }
            }
        ...
        public static string FindExtMimetype(string ext, string defaultmimetype = "text/plain")
        {
            ...
            if (ext.LastIndexOf(".") >= 0)
            {
                ...
                {
                    TextExtension(ext);
                    lock (mappedExtentions)
                    {
                        if (mappedExtentions.ContainsKey(ext))
                        {
                            mimetype = mappedExtentions[ext];
                        }
                        else
                        {
                            var mimestream = new System.IO.StringReader(mimetypesvls);
                            var mline = mimestream.ReadLine();
                            if (mline.Equals(""))
                            {
                                mappedExtentions.Add(ext, mimetype);
                            }
                            else
                            {
                                while (mline != null && !mline.Equals(""))
                                {
                                    if (mline.Contains("|"))
                                    {
                                        if (mline.Split("|".ToCharArray())[2].Equals(ext))
                                        {
                                            mimetype = mline.Split("|".ToCharArray())[1];
                                            if (!mappedExtentions.ContainsKey(ext))
                                            {
                                                mappedExtentions.Add(ext, mimetype);
                                            }
