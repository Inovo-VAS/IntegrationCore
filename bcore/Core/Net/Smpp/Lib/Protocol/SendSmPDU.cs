...
namespace Lnksnk.Core.Net.Smpp.Lib.Protocol
{
    public abstract class SendSmPDU : SmPDU
    {
        ...
        public virtual void GetMessageText(out string message, out Udh udh)
        {
            ...
            {
                Log.LOG().Info("200020:UDH field presense detected;");
                ...
                {
                    Log.LOG().ErrorFormat("20023:UDH field parsing error - {0}", ex, new ByteBuffer(msgBytes).DumpString());
            ...
            {
                Log.LOG().ErrorFormat("200019:SMS message decoding failure - {0}", ex1, new ByteBuffer(msgBytes).DumpString());
                if (vTraceSwitch.TraceError)
                {
                    Trace.WriteLine(string.Format(
