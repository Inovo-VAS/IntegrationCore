    ...
    public sealed class With : CodeNode
    {
        ...
        internal static CodeNode Parse(ParseInfo state, ref int index)
        {
            ...
            if (!Parser.Validate(state.Code, "with (", ref i) && !Parser.Validate(state.Code, "with(", ref i))
                return null;
            if (state.Strict)
                ExceptionHelper.Throw((new Bcoring.ES6.BaseLibrary.SyntaxError("WithStatement is not allowed in strict mode.")));
            ...
            while (Tools.IsWhiteSpace(state.Code[i]))
                i++;
            if (state.Code[i] != ')')
                ExceptionHelper.Throw((new Bcoring.ES6.BaseLibrary.SyntaxError("Invalid syntax WithStatement.")));
            do
                i++;
            while (Tools.IsWhiteSpace(state.Code[i]));
