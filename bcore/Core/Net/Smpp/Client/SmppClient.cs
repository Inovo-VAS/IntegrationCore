...
using Lnksnk.Core.Net.Smpp.Lib.Util;
using Lnksnk.Core.Net.Smpp.Lib.Protocol.Tlv;
using Lnksnk.Core.Net.Smpp.Lib.Logging;
using Lnksnk.Core.Logging;

namespace Lnksnk.Core.Net.Smpp.Client
{
    //public delegate void MessagingEvent(object sender, MessageEventArgs e,ShortMessage message,string sourceAddress,string destinationAddress);
    ...
    public class SmppClient : IDisposable
    {
        ...
        //public MessagingEvent MessageReceived;
        ...
        public void SendMessage(ShortMessage message, int timeOut)
        {
            ...
            {
                if (Log.LOG().IsDebugEnabled) Log.LOG().DebugFormat("SendMessage SendSmPDU: {0}", LoggingExtensions.DumpString(pdu, vSmppEncodingService));
                ResponsePDU resp = SendPdu(pdu, timeOut);
                var submitSmResp = resp as SubmitSmResp;
                if (submitSmResp != null)
                {
                    if (Log.LOG().IsDebugEnabled) Log.LOG().DebugFormat("SendMessage Response: {0}", LoggingExtensions.DumpString(resp, vSmppEncodingService));
        ...
        private void Open(int timeOut)
        {
            try
            {
                if (Monitor.TryEnter(vConnSyncRoot))
                {
                    ...
                    {
                        Log.LOG().ErrorFormat("OpenSession: {0}", ex, ex.Message);
        ...
        private void PduReceivedEventHander(object sender, PduReceivedEventArgs e)
        {
            ...
            if (Log.LOG().IsDebugEnabled)
                Log.LOG().DebugFormat("Received PDU: {0}", LoggingExtensions.DumpString(pdu, vSmppEncodingService));
            ...
            {
                Log.LOG().ErrorFormat("200019:SMPP message decoding failure - {0} - {1} {2}", smppEx, smppEx.ErrorCode, new ByteBuffer(pdu.GetBytes()).DumpString(), smppEx.Message);
            ...
            {
                Log.LOG().ErrorFormat("200019:SMPP message decoding failure - {0}", ex, new ByteBuffer(pdu.GetBytes()).DumpString());
                if (vTraceSwitch.TraceError)
                {
                    Trace.WriteLine(string.Format(
            ...
            if (message != null && Log.LOG().IsDebugEnabled)
                Log.LOG().DebugFormat("PduReceived: message: {0}", LoggingExtensions.DumpString(message, vSmppEncodingService));
