    ...
    public sealed class ObjectDesctructor : Expression
    {
            ...
            private Arguments assign(JSValue target, JSValue value, object targetName, Arguments setterArgs)
            {
                if (target._valueType == JSValueType.Property)
                {
                    ...
                    if (setter != null)
                        ...
                        ExceptionHelper.ThrowTypeError(string.Format(Bcoring.ES6.JS.CannotAssignReadOnly, targetName));
                }
                else
                {
        ...
        public static bool CheckObjectDefinition(ArrayDefinition arrayDefinition, bool @throw)
        {
            ...
            for (var i = 0; i < arrayDefinition.Elements.Length; i++)
            {
                ...
                {
                    if (@throw)
                        ExceptionHelper.ThrowReferenceError(Bcoring.ES6.JS.InvalidLefthandSideInAssignment);
                    return false;
                }
            }
