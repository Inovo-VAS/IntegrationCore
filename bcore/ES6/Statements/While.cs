...
namespace Bcoring.ES6.Statements
{
#if !(PORTABLE || NETCORE)
    [Serializable]
#endif
    public sealed class While : CodeNode
    {
        ...
        internal static CodeNode Parse(ParseInfo state, ref int index)
        {
            ...
            if (i >= state.Code.Length)
                ExceptionHelper.Throw(new SyntaxError(Bcoring.ES6.JS.UnexpectedEndOfSource));
            if (state.Code[i] != ')')
            ...
            if (body is FunctionDefinition)
            {
                if (state.Message != null)
                    state.Message(MessageLevel.CriticalWarning, body.Position, body.Length, Bcoring.ES6.JS.DoNotDeclareFunctionInNestedBlocks);
