    ...
    public sealed class ExpressionTree : Expression
    {
        ...
        private static Expression parseContinuation(ParseInfo state, Expression first, int startIndex, ref int i, ref bool root, bool forUnary, bool processComma, bool forNew, bool forForLoop)
        {
                ...
                switch (state.Code[i])
                {
                    ...
                    case '.':
                    {
                        ...
                        if (!Parser.ValidateName(state.Code, ref i, false, true, state.Strict))
                            ExceptionHelper.Throw(new SyntaxError(string.Format(Bcoring.ES6.JS.InvalidPropertyName, CodeCoordinates.FromTextPosition(state.Code, i, 0))));
                    ...
                    case '(':
                    {
                        ...
                        for (; ; )
                        {
                            ...
                            if (i + 1 == state.Code.Length)
                                ExceptionHelper.ThrowSyntaxError(Bcoring.ES6.JS.UnexpectedEndOfSource, state.Code, i);
            ...
            if ((kind == OperationType.Assignment) || assign)
            {
                ...
                if (error)
                    ExceptionHelper.ThrowReferenceError(Bcoring.ES6.JS.InvalidLefthandSideInAssignment, state.Code, first.Position, first.Length);
            }
                ...
                if (!root || kind != OperationType.None || second != null)
                {
                    ...
                    else
                    {
                        if (kind == OperationType.NullishCoalescing)
                        {
                            ...
                            {
                                ExceptionHelper.ThrowSyntaxError(Bcoring.ES6.JS.LogicalNullishCoalescing, state.Code, startIndex, i - startIndex);
                            }
                        }
        ...
        private static Expression parseOperand(ParseInfo state, ref int i, bool forNew, bool forForLoop)
        {
                ...
                else if (Parser.ValidateValue(state.Code, ref i))
                {
                    ...
                    else
                    {
                        ...
                        if (value == "null")
                            ...
                            operand = new Constant(b ? Bcoring.ES6.BaseLibrary.Boolean.True : Bcoring.ES6.BaseLibrary.Boolean.False) { Position = start, Length = i - start };
                        else
                        {
                            int n = 0;
                            double d = 0;
                        ...
                        case '+':
                        {
                            ...
                            if (state.Code[i] == '+')
                            {
                                ...
                                if (i >= state.Code.Length)
                                    ExceptionHelper.ThrowSyntaxError(Bcoring.ES6.JS.UnexpectedEndOfSource);
            ...
            if (operand == null || operand is Empty)
                ExceptionHelper.ThrowSyntaxError(Bcoring.ES6.JS.UnexpectedToken, state.Code, i);
