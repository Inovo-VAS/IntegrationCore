...
using Lnksnk.Core.Net.Smpp.Lib.Networking;
...
#endif
namespace Lnksnk.Core.Net.Smpp.Lib
{
    /// <summary>
        ...
        protected override void RunNow()
        {
            while (CanContinue())
            {
                try
                {
                    ...
                    {
#if NETCOREAPP3_1
                        Task.Run(() => vProcessorCallback((RequestPDU)pdu)).Wait();
#else
                ...
                {
                    Log.LOG().ErrorFormat("200015:Unanticipated stream parser exception: {0}", ex, ex.Message);
...
#region Helper Methods
        private PDU WaitPDU()
        {
            ...
            {
                Log.LOG().ErrorFormat("200010:TCP/IP Exception encountered while reading pdu header bytes: {0}", tcpIp_ex_1, tcpIp_ex_1.Message);
            ...
            Log.LOG().TraceFormat("PDU header: {0}", Logging.LoggingExtensions.DumpString(header, vSmppEncodingService));
            ...
            catch (InvalidPDUCommandException inv_ex)
            {
                ...
                Log.LOG().WarnFormat("200011:Invalid PDU command type:{0};", iBuffer.DumpString());
            ...
            {
                Log.LOG().ErrorFormat("200012:TCP/IP Exception encountered while reading pdu body bytes: {0}", tpcIp_ex_2, tpcIp_ex_2.Message);
                if (vTraceSwitch.TraceInfo)
                {
                    Trace.WriteLine(string.Format(
            ...
            catch (PDUException pdu_ex)
            {
                ...
                Log.LOG().WarnFormat("200013:Malformed PDU body received:{0}", pdu_ex, pBuffer.DumpString());
                if (vTraceSwitch.TraceWarning)
                {
                    Trace.WriteLine(string.Format(
        ...
        private byte[] WaitBytes(int length)
        {
            ...
            while (remaining > 0)
            {
                ...
                if (receiveCount == 0)
                    Log.LOG().Warn("200014:TCP/IP receive operation returned zero bytes;");
