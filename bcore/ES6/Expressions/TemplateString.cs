...
namespace Bcoring.ES6.Expressions
{
    ...
    public sealed class TemplateString : Expression
    {
        ...
        {
            if (state.Code[index] != '`')
                ExceptionHelper.ThrowSyntaxError(Bcoring.ES6.JS.UnexpectedToken, state.Code, index);
            ...
            for (;;)
            {
                ...
                if (state.Code[pos] == '$')
                {
                    ...
                    if (state.Code[pos] != '}')
                        ExceptionHelper.ThrowSyntaxError(Bcoring.ES6.JS.UnexpectedToken, state.Code, pos);
                }
